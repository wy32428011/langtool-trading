...
def show_active_markets(markets: List[Dict[str, Any]]):
    ...
    # 按结束日期排序
    def get_end_date(m):
        return m.get("endDateISO") or m.get("endDateIso") or "9999-12-31T23:59:59Z"

    active_markets.sort(key=get_end_date)

    print("\n" + "="*120)
    print(f"Polymarket Active Markets Report ({datetime.now().strftime('%Y-%m-%d %H:%M:%S')})")
    print("-" * 120)
